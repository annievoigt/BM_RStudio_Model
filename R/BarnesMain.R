...
# load data files
all_files <- list.files("data", pattern = ".csv")
...
exp_data <- rbindlist(lapply(X = all_files, FUN = function(file_name) {
  tmp_dt <- fread(paste0("data/", file_name))
  split_string <- unlist(strsplit(file_name, split = "_"))
  conditions_string <- split_string[5]
...
setkeyv(x = exp_data, c("Round", "Day", "Trial","frame"))


exp_data[,`:=`(hole1=hole1+1,hole2=hole2+1),] # remove when data offset is corrected in files

# transform data to radians and x y coordinates on a unit circle
exp_data[,`:=`(target_rad = target_hole * pi / 20 * 2 - pi/20 -pi),][,`:=`(x_target = sin(target_rad), y_target = -cos(target_rad)),]
exp_data[,`:=`(hole2_rad = hole2 * pi / 20 * 2 - pi/20 -pi - pi, hole1_rad = hole1 * pi / 20 * 2 - pi/20 -pi),]
exp_data[,`:=`(x1 = sin(hole1_rad), y1 = -cos(hole1_rad), x2 = sin(hole2_rad), y2 = -cos(hole2_rad)),]

# define starting point as 0,0 (center of unit circle)
exp_data[hole2==0,`:=`(x2=0,y2=0),]
...
exp_data[,target_hole:=factor(target_hole),][
  ,Day:=factor(x = Day, levels=c("1", "2", "3", "4", "5", "12")),]

# Plot Barnes Maze data

# plot overview of Barnes Maze data as graphs
ggplot(data = exp_data, aes(x = x1, y = y1, color = frame, group = interaction(MouseID, TrialUnique))) +
  geom_point() +
...
model_formula <- brmsformula(hole1_rad ~ x2*y2 + s(target_distance) + s(previous_target_distance) + Trial  + (Trial|Day) + target_hole,
...
fit_prior <- brm(formula = model_formula, data = exp_data, family = von_mises(),
                 prior = new_priors, chains = 4, iter = 2000, warmup = 2000,
...
# plot posterior predictive checks of hole number
bayesplot::ppc_dens_overlay_grouped(y = exp_data$hole1, yrep = y_rep_hole[1:50,], group = exp_data$Round, bw = .1)+
...
bayesplot::ppc_intervals_grouped(y = exp_data$hole1_rad, yrep = y_rep, group = exp_data$Round)
